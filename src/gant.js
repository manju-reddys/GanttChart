define('ganttChart',
    ['jquery'], //Loading dependencies here
    // module definition function
    // dependencies are mapped to function parameters


    function ($) {
        var ganttChart = function (options) {
            options = options || {};
            ...
            var globalConfig = (function (def) {
                ...
                if ($.isEmptyObject(def) || $.isFunction(def)) {
                    ...
                    delete def.langs;
                    return $.extend(true, {}, defaultConfig, def);
                }
            })(options);
            ...
            var helper = {
                 ...
                 */
                getUUID: function (d) {
                    return (Math.random().toString(16) + '000000000').substr(2, d || 4);
                },
                ...
                toDate: function (string) {
                    ...
                    var date = new Date('2011-06-02T09:34:29+02:00');//Some known date
                    if (!date || +date !== 1307000069000) { //ISO date not supported
                        var day, tz,
            ...
            function Create($element, format, ganttId, config) {
                //Variables definition
                var taskList = [], minGpLen = 8, scrollTo = '', captionType;

                $element.html(getHtmlTemplate(ganttId)); //Clean and attach the barebone structure
                    ...
                    config: function () {
                        return {
                            ...
                            setHourColWidth: function (width) {
                                config.hourColWidth = parseInt(width, 10);
                                return this;
                            },
                            setDayColWidth: function (width) {
                                config.dayColWidth = parseInt(width, 10);
                                return this;
                            },
                            setWeekColWidth: function (width) {
                                config.weekColWidth = parseInt(width, 10);
                                return this;
                            },
                            setMonthColWidth: function (width) {
                                config.monthColWidth = parseInt(width, 10);
                                return this;
                            },
                            setQuarterColWidth: function (width) {
                                config.quarterColWidth = parseInt(width, 10);
                                return this;
                            },
                            setRowHeight: function (height) {
                                config.rowHeight = parseInt(height, 10);
                                return this;
                            },
            ...
            return {
                ...
                newInstance: function ($element, format) {
                    ...
                    config = $.extend(true, {}, globalConfig);

                    format = $.inArray(format, config.viewTypes) !== -1 ? format : 'day';

                    pGantt = new Create($element, format, ganttId, config);
                    instances[ganttId] = pGantt;
                    ...
                    function getOffset(startDate, endDate, colWidth, $format) {
                        ...
                        var monthDays = config.monthDays,
                            ...
                            taskRight = (tmpTaskEnd - tmpTaskStart) / 3600000; // Length of task in hours
                        ...
                        if ($format === 'day') {
                            ...
                            var monthsDiff = (12 * (taskEnd.getFullYear() - taskStart.getFullYear())) + (taskEnd.getMonth() - taskStart.getMonth()),
                                posTmpDate = new Date(taskEnd.getTime());
                            posTmpDate.setDate(taskStart.getDate());
                            var daysCrctn = (taskEnd.getTime() - posTmpDate.getTime()) / (86400000);
                            taskRightPx = Math.ceil((monthsDiff * (colWidth + 1)) + (daysCrctn * (colWidth / monthDays[taskEnd.getMonth()])));
                            ...
                            taskRightPx = Math.ceil((monthsDiff * ((colWidth + 1) / 3)) + (daysCrctn * (colWidth / 90)));
                        } else if ($format === 'hour') {
                    ...
                    pGantt.calcTaskXY = function () {
                        var list = pGantt.getTaskList(),
                    ...
                    pGantt.addTaskItem = function (taskItem) {
                        var isExist = false,
                            list = pGantt.getTaskList();
                        for (var i = 0; i < list.length; i++) {
                            var task = list[i];
                    ...
                    pGantt.removeTaskItem = function (id) {
                    ...
                    pGantt.clearDependencies = function () {
                     ...
                     * @param cssClass
                     */
                    pGantt.sLine = function (x1, y1, x2, y2, cssClass) {
                     ...
                     * @param cssClass
                     */
                    pGantt.dLine = function (x1, y1, x2, y2, cssClass) {
                    ...
                    pGantt.drawDependency = function (x1, y1, x2, y2, type, cssClass) {
                    ...
                    pGantt.drawDependencies = function () {
                    ...
                    pGantt.renderGridHeader = function () {
                    ...
                    pGantt.renderGridBody = function () {
                    ...
                    pGantt.renderChartHeader = function (vTmpDate, vMinDate, vMaxDate, vColWidth) {
                    ...
                    pGantt.renderChartBody = function (vTaskLeftPx, vTaskRightPx, vColWidth, vMinDate, vMaxDate, vNumCols, vNumRows) {


                        return {vTaskLeftPx: vTaskLeftPx, vTaskRightPx: vTaskRightPx, vColWidth: vColWidth};
                    };

                    pGantt.render = function () {
                        ...
                        if (tasksCount > 0) {
                            ...
                            this.getGridBody().on('scroll', function (ev) {
                                pGantt.getChartBody().scrollTop(ev.target.scrollTop);
                            });

                            this.getChartBody().on('scroll', function (ev) {
                                pGantt.getGridBody().scrollTop(ev.target.scrollTop);
                                pGantt.getChartHeader().scrollLeft(ev.target.scrollLeft);
                            });

                            this.getChartHeader().on('scroll', function (ev) {
                                pGantt.getChartBody().scrollLeft(ev.target.scrollLeft);
                            });
                            $(window).resize(function () {
                                pGantt.getChartHeader().scrollLeft(pGantt.getChartBody().scrollLeft());
                                pGantt.getGridBody().scrollTop(pGantt.getChartBody().scrollTop());
                            });
                    ...
                    pGantt.addFolderListeners = function ($ele, id) {
                        var self = this;
                    ...
                    pGantt.getWidth = function (width) {
                        return isNaN(width * 1) ? width : width + 'px';
                    };

                    pGantt.getCaption = function (task) {
                        var caption = '';
                    ...
                    pGantt.buildInformationTableData = function (descriptions, numRows) {
                        var tasksList = this.getTaskList(),
                            tasksCount = tasksList.length;
                        ...
                        for (var i = 0; i < tasksCount; i++) {
                            ...
                            if (task.getGroup() === 2 || !(task.getParItem() && task.getParItem().getGroup() === 2)) {
                                var cellContent = '';
                                entry['id'] = this.getId() + 'child_' + taskId;
                                entry['rowClass'] = 'gname ' + BGColor;
                                if (task.getVisible()) {
                                    entry['display'] = 'block';
                                } else {
                                    entry['display'] = 'none';
                                ...
                                description.push({cssClass: 'gtasklist', text: '\u00A0'});
                                description.push({cssClass: 'gtaskname', text: ''});

                                for (var j = 0; j < task.getLevel(); j++) {
                                    cellContent += '\u00A0\u00A0\u00A0\u00A0';
                                }
                                ...
                                if (task.getGroup() === 1) {
                                    ...
                                    entry['text'] = cellContent +
                                        '<span id="' + this.getId() + 'group_' + taskId + '" class="gfoldercollapse">' + task.getOpen() ? '-' : '+' + '</span>' +
                                        '\u00A0' + task.getName();
                                } else {
                                    cellContent += '\u00A0\u00A0\u00A0\u00A0';
                                    entry['text'] = cellContent + task.getName();
                                }

                                description.push({cssClass: 'gresource ' + this.getShowResource() ? '' : 'hidden', text: task.getResource()});
                                description.push({cssClass: 'gduration ' + this.getShowDuration() ? '' : 'hidden', text: task.getDuration(format, config.langs[config.lang])});
                                description.push({cssClass: 'gpccomplete ' + this.getShowCompletion() ? '' : 'hidden', text: task.getCompletion()});
                                description.push({cssClass: 'gstartdate ' + this.getShowStartDate() ? '' : 'hidden', text: this.formatDate(task.getStartDate(), this.getDateTaskTableDisplayFormat())});
                                description.push({cssClass: 'genddate ' + this.getShowEndDate() ? '' : 'hidden', text: this.formatDate(task.getEndDate(), this.getDateTaskTableDisplayFormat())});
                    ...
                    pGantt.formatDate = function (date, displayFormat) {
                        var lang = config.langs[config.lang],
                    ...
                    pGantt.getIsoWeek = function (date) {
                     ...
                     * @returns {*} String|Date
                     */
                    pGantt.date = function (date, $format) {
                        ...
                        $format = $format ? $format
                            ...
                            .replace('mm', 'm')
                            .replace('MM', 'F') : false;
                        return $format ? date(date).format($format) : date(date);
                    };

                    pGantt.drawSelector = function () {
                    ...
                    pGantt.processRows = function (list, processId, row, level, open, useSort) {
                    ...
                    pGantt.sortTasks = function (list, id, idx) {
                     ...
                     * @returns {Date}
                     */
                    pGantt.getMinDate = function getMinDate(taskList, $format) {
                        var date = new Date();
                     ...
                     * @returns {Date}
                     */
                    pGantt.getMaxDate = function (taskList, $format) {
                        var date = new Date();
                    ...
                    pGantt.newTaskItem = function (id, name, startDate, endDate, cssClass, completion, parent, open, group) {
                        ...
                        if (vGroup !== 1) {
                            vStart = this.date(startDate, this.getDateInputFormat());
                            vEnd = this.date(endDate, this.getDateInputFormat());
                        }
                        ...
                        return {
                            ...
                            setStartDate: function (date) {
                                vStart = this.date(date, this.getDateInputFormat());
                                return this;
                            },
                            setEndDate: function (date) {
                                vEnd = this.date(date, this.getDateInputFormat());
                                return this;
                            },
                            ...
                            getDuration: function () {
                                ...
                                if (vMile) {
                                    ...
                                    var tempPer = pGantt.getOffset(this.getStart(), taskEnd, 999, units) / 1000;
                    ...
                    pGantt.addTasks = function (tasks) {
                        ...
                        for (var t = 0; t < n; t++) {
                            ...
                            if (item.id !== 0) {
                                ...
                                task = new this.newTaskItem(item.id, item.name, item.start, item.end, item.class,
                                    item.completion, item.parent, item.open, item.open);
                                task.setLink(item.link)
                                    .setMile(item.mile)
                                    .setResource(item.resource)
                                    .setCompletion(item.complete)
                                    .setDepend(item.depend)
